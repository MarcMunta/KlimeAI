...
if not defined PS_EXE (
	echo [logs] ERROR: PowerShell not found in PATH.
	echo [logs] Install PowerShell 7: pwsh.exe, or Windows PowerShell: powershell.exe.
	exit /b 1
)

%PS_EXE% -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%ROOT%logs_cli.ps1" %*
exit /b %errorlevel%
