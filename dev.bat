...
if not defined PS_EXE (
	echo [dev] ERROR: PowerShell not found in PATH.
	echo [dev] Install PowerShell 7: pwsh.exe, or Windows PowerShell: powershell.exe.
	exit /b 1
)

%PS_EXE% -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%ROOT%dev_cli.ps1" %*
exit /b %errorlevel%
